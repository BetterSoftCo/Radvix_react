...
export class RemoteEquipment {
  ...
  getAllEquipments(body: { PageNumber: number; PageSize: number, SearchParameter: string }, action: (res: GetAllEquipment) => any, error: (res: any) => any) {
    return HTTP.get(`/Equipment?PageSize=${body.PageSize}&PageNumber=${body.PageNumber}&SearchParameter=${body.SearchParameter}`).then((res) => action(res.data)).catch((err) => { error(err) });
  }
